...
import rv32i_types::*;
#(
    parameter RES_DIV_ENTRIES = 5,      // Width of data bus
...
(
    ...
    output logic rs_div_full,

    // For CDB
    input cdb_t cdb_in_add, cdb_in_mul, cdb_in_div, cdb_in_ls,

    //output to send to physical reg file
    output logic [5:0] ps1_div_tosend_tophyreg, ps2_div_tosend_tophyreg,
    // output logic [31:0] imm_rs2_val
    output id_ex_stage_reg_t decode_packet_from_div_rs,
    output logic [$clog2(QUEUE_DEPTH_ROB)-1:0] rob_entry_from_div_rs,
    output logic [5:0] pd_from_div_rs,
    output logic activate_div,
    ...
    localparam  RES_ENTRIES_BITS = $clog2(RES_DIV_ENTRIES);
    reservation_station_t res_array [RES_DIV_ENTRIES];
    logic busy [RES_DIV_ENTRIES];
    ...
    always_comb begin
        rs_div_full = 1'b1;
        for (int i = 0; i < RES_DIV_ENTRIES; i++) begin
            if (busy[i] == '0) rs_div_full = 1'b0;
        end
    end
    ...
    always_comb begin
        ...
        busy_next = 'x;
        index_next = 'x;        
        for (int unsigned i = 0; i < RES_DIV_ENTRIES; i++) begin
            if (busy[i] == 0) begin
    ...
    always_comb begin 
        ps2_div_tosend_tophyreg = '0;
        ps1_div_tosend_tophyreg = '0;
        decode_packet_from_div_rs = '0;
        activate_div = '0;
        remove_idx = '0;
        rob_entry_from_div_rs = '0;
        pd_from_div_rs = '0;
        for (int unsigned i = 0; i < RES_DIV_ENTRIES; i++) begin
            if ((counter == '0) && busy[i] && res_array[i].ps1_v_res && res_array[i].ps2_v_res && !flush) begin
                ps2_div_tosend_tophyreg = res_array[i].ps2_res;
                ps1_div_tosend_tophyreg = res_array[i].ps1_res;
                decode_packet_from_div_rs = res_array[i].decode_packet;
                rob_entry_from_div_rs = res_array[i].rob_entry_res;
                pd_from_div_rs = res_array[i].pd_res;
                activate_div = 1'b1;
    ...
    always_ff @(posedge clk) begin
        if (rst || flush) begin
            ...
            if (activate_div) busy[remove_idx] <= 1'b0;
        end
    end
